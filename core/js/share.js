...
OC.Share={
	...
	showDropDown:function(itemType, itemSource, appendTo, link, possiblePermissions, filename) {
		...
		if (possiblePermissions & OC.PERMISSION_SHARE) {
			...
			html += '<div id="maxHelp" style="font-size: 80%; color: #666;font-style: italic; text-align: center">' + t('core', 'For your information, the results list is limited to 15 items.') + '</div>';

			html += '<ul id="shareWithList">';
			html += '</ul>';

			html += '<div id="fakeval" style="display: none;padding-bottom: 8px;text-align: center;"><button>' + t('core', 'Share') + '</button></div>';
	...
	addShareWith:function(shareType, shareWith, shareWithDisplayName, permissions, possiblePermissions, mailSend, collection) {
		...
		if (collection) {
			...
			if (possiblePermissions & OC.PERMISSION_CREATE || possiblePermissions & OC.PERMISSION_UPDATE || possiblePermissions & OC.PERMISSION_DELETE) {
				html += '<input id="canEdit-'+escapeHTML(shareWith)+'" type="checkbox" name="edit" class="permissions" '+editChecked+' disabled="disabled"/><label for="canEdit-'+escapeHTML(shareWith)+'">'+t('core', 'can edit')+'</label>';
			}
			...
			html += '<div class="cruds" style="display:none;">';
				if (possiblePermissions & OC.PERMISSION_CREATE) {
					html += '<input id="canCreate-'+escapeHTML(shareWith)+'" type="checkbox" name="create" class="permissions" '+createChecked+' data-permissions="'+OC.PERMISSION_CREATE+'" disabled="disabled"/><label for="canCreate-'+escapeHTML(shareWith)+'">'+t('core', 'create')+'</label>';
				}
				if (possiblePermissions & OC.PERMISSION_UPDATE) {
					html += '<input id="canUpdate-'+escapeHTML(shareWith)+'" type="checkbox" name="update" class="permissions" '+updateChecked+' data-permissions="'+OC.PERMISSION_UPDATE+'" disabled="disabled"/><label for="canUpdate-'+escapeHTML(shareWith)+'">'+t('core', 'update')+'</label>';
				}
				if (possiblePermissions & OC.PERMISSION_DELETE) {
					html += '<input id="canDelete-'+escapeHTML(shareWith)+'" type="checkbox" name="delete" class="permissions" '+deleteChecked+' data-permissions="'+OC.PERMISSION_DELETE+'" disabled="disabled"/><label for="canDelete-'+escapeHTML(shareWith)+'">'+t('core', 'delete')+'</label>';
				}
			html += '</div>';
			html += '</li>';
...
$(document).ready(function() {
	...
	$(document).on('click', '#fakeval', function() {
		...
		OC.Notification.show('Modifications enregistrées');
		if (OC.Share.notificationTimeout){
			window.clearTimeout(OC.Share.notificationTimeout);
		}
		OC.Share.notificationTimeout = window.setTimeout(
			function(){
				OC.Notification.hide();
				OC.Share.notificationTimeout = null;
			}, 10000);
	});
